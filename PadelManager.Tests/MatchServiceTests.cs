...
public class MatchServiceTests {
    ...
    public MatchServiceTests() {
        ...
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("G001")).ReturnsAsync(MembreValide("G001", "GLOBAL", null, 21));
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("L001")).ReturnsAsync(MembreValide("L001", "LIBRE", null, 5));
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("L002")).ReturnsAsync(MembreValide("L002", "LIBRE", null, 5));
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("L003")).ReturnsAsync(MembreValide("L003", "LIBRE", null, 5));
    ...
    private static CreerMatchPriveRequestDto RequeteValide() => new() {
        ...
        Joueurs = new List<string> { "L001", "L002", "L003" }
    };
    ...
    public async Task CreerMatchPriveAsync_RequeteValide_CreeLeMatchAvecParticipationEtPaiementOrganisateurEtTroisJoueurs() {
        ...
        Assert.Equal(new[] { "G001", "L001", "L002", "L003" }, resultat.Match.Joueurs);

        _matchRepoMock.Verify(r => r.AddAsync(It.Is<Match>(m =>
            m.SiteId == 1 && m.TerrainId == 11 && m.OrganisateurMatricule == "G001" &&
            m.Statut == "INCOMPLET" && m.Visibilite == "PRIVE" &&
            m.Participations.Count == 4 &&
            m.Participations.Single(p => p.MembreMatricule == "G001").Paiement != null &&
            m.Participations.Single(p => p.MembreMatricule == "G001").Paiement!.MontantParticipation == 15.00m &&
            m.Participations.Single(p => p.MembreMatricule == "L001").Paiement == null &&
            m.Participations.Single(p => p.MembreMatricule == "L002").Paiement == null &&
            m.Participations.Single(p => p.MembreMatricule == "L003").Paiement == null)), Times.Once);
    }
    ...
    public async Task CreerMatchPriveAsync_MembreSiteSurUnAutreSite_RetourneEchec() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("S003")).ReturnsAsync(MembreValide("S003", "SITE", 2, 14));
        var requete = RequeteValide();
        requete.OrganisateurMatricule = "S003"; // rattaché au site 2, match sur le site 1
    ...
    public async Task CreerMatchPriveAsync_MembreSiteSurSonPropreSite_Autorise() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("S001")).ReturnsAsync(MembreValide("S001", "SITE", 1, 14));
        var requete = RequeteValide();
        requete.OrganisateurMatricule = "S001";
    ...
    public async Task CreerMatchPriveAsync_PenaliteActive_RetourneEchecMemePourUneDateEloignee() {
        _penaliteRepoMock.Setup(r => r.GetPlusRecenteAsync("G001"))
            .ReturnsAsync(new Penalite { MembreMatricule = "G001", MatchOrigineId = 1, DelaiJusquAu = Aujourdhui.AddDays(7) });
        var requete = RequeteValide();
    ...
    public async Task CreerMatchPriveAsync_PenaliteExpiree_Autorise() {
        _penaliteRepoMock.Setup(r => r.GetPlusRecenteAsync("G001"))
            .ReturnsAsync(new Penalite { MembreMatricule = "G001", MatchOrigineId = 1, DelaiJusquAu = Aujourdhui.AddDays(-1) });
    ...
    public async Task CreerMatchPriveAsync_PlusDeTroisJoueurs_RetourneEchec() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync(It.Is<string>(m => m != "G001"))).ReturnsAsync(MembreValide("X", "LIBRE", null, 5));
        var requete = RequeteValide();
        requete.Joueurs = new List<string> { "L001", "L002", "L003", "L004" };
    ...
    public async Task CreerMatchPriveAsync_MoinsDeTroisJoueurs_RetourneEchec(int nombreJoueurs) {
        var requete = RequeteValide();
        requete.Joueurs = new List<string> { "L001", "L002", "L003" }.Take(nombreJoueurs).ToList();
    ...
    public async Task CreerMatchPriveAsync_JoueurDuplique_RetourneEchec() {
        var requete = RequeteValide();
        requete.Joueurs = new List<string> { "L001", "L001", "L002" };
    ...
    public async Task CreerMatchPriveAsync_JoueurEstLOrganisateur_RetourneEchec() {
        var requete = RequeteValide();
        requete.Joueurs = new List<string> { "G001", "L001", "L002" };
    ...
    public async Task CreerMatchPriveAsync_JoueurIntrouvable_RetourneEchec() {
        ...
        var requete = RequeteValide();
        requete.Joueurs = new List<string> { "INCONNU", "L001", "L002" };
    ...
    public async Task CreerMatchPublicAsync_RequeteValide_CreeLeMatchAvecSeuleLaParticipationDeLOrganisateur() {
        ...
        Assert.Equal("INCOMPLET", resultat.Match.Statut);
        Assert.Equal(new[] { "G001" }, resultat.Match.Joueurs);

        _matchRepoMock.Verify(r => r.AddAsync(It.Is<Match>(m =>
            m.Visibilite == "PUBLIC" && m.OrganisateurMatricule == "G001" &&
    ...
    public async Task CreerMatchPublicAsync_MembreSiteSurUnAutreSite_RetourneEchec() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("S003")).ReturnsAsync(MembreValide("S003", "SITE", 2, 14));
        var requete = RequeteValidePublic();
        requete.OrganisateurMatricule = "S003";
    ...
    public async Task ObtenirCreneauxDisponiblesAsync_CroiseDisponibiliteEtTerrainsEnExcluantLesMatchsPris() {
        ...
        _matchRepoMock.Setup(r => r.GetForSiteAndDateAsync(1, date)).ReturnsAsync(new List<Match> {
            new() { SiteId = 1, TerrainId = 11, DateHeure = date.ToDateTime(new TimeOnly(9, 0)), Visibilite = "PRIVE", OrganisateurMatricule = "G001", Statut = "INCOMPLET" }
        });
    ...
    private static Match MatchPublic(int id, int siteId, DateTime dateHeure, params string[] participants) {
        var match = new Match {
            ...
            DateHeure = dateHeure,
            Visibilite = "PUBLIC",
            OrganisateurMatricule = participants.Length > 0 ? participants[0] : "G001",
            Statut = "INCOMPLET",
    ...
    public async Task ObtenirMatchsPublicsAsync_MembreGlobal_VoitTousLesSites() {
        _matchRepoMock.Setup(r => r.GetPublicsIncompletsAsync(It.IsAny<DateTime>())).ReturnsAsync(new List<Match> {
            MatchPublic(1, 1, DateTime.Now.AddDays(1), "L001"),
            MatchPublic(2, 2, DateTime.Now.AddDays(20), "L001")
        });

        var resultat = await _service.ObtenirMatchsPublicsAsync("G001");
    ...
    public async Task ObtenirMatchsPublicsAsync_MembreSite_NeVoitQueSonSite() {
        ...
        _matchRepoMock.Setup(r => r.GetPublicsIncompletsAsync(It.IsAny<DateTime>())).ReturnsAsync(new List<Match> {
            MatchPublic(1, 1, DateTime.Now.AddDays(1), "L001"),
            MatchPublic(2, 2, DateTime.Now.AddDays(1), "L001")
        });

        var resultat = await _service.ObtenirMatchsPublicsAsync("S001");
    ...
    public async Task ObtenirMatchsPublicsAsync_MembreLibre_VoitSansLimiteDeDelai() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("L001")).ReturnsAsync(MembreValide("L001", "LIBRE", null, 5));
        _matchRepoMock.Setup(r => r.GetPublicsIncompletsAsync(It.IsAny<DateTime>())).ReturnsAsync(new List<Match> {
            MatchPublic(1, 1, DateTime.Now.AddDays(5), "G001"),
            MatchPublic(2, 1, DateTime.Now.AddDays(60), "G001")
        });

        var resultat = await _service.ObtenirMatchsPublicsAsync("L001");
    ...
    public async Task ObtenirMatchsPublicsAsync_MembreDejaInscrit_NeLeVoitPlus() {
        _matchRepoMock.Setup(r => r.GetPublicsIncompletsAsync(It.IsAny<DateTime>())).ReturnsAsync(new List<Match> {
            MatchPublic(1, 1, DateTime.Now.AddDays(1), "G001") // G001 déjà organisateur/participant
        });
    ...
    public async Task ObtenirMatchsPublicsAsync_CalculeLesPlacesRestantes() {
        _matchRepoMock.Setup(r => r.GetPublicsIncompletsAsync(It.IsAny<DateTime>())).ReturnsAsync(new List<Match> {
            MatchPublic(1, 1, DateTime.Now.AddDays(1), "L001", "L002")
        });
    ...
    public async Task RejoindreMatchPublicAsync_MatchIntrouvable_RetourneEchec() {
        ...
        var resultat = await _service.RejoindreMatchPublicAsync(99, "G001");
    ...
    public async Task RejoindreMatchPublicAsync_MatchPrive_RetourneEchec() {
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(1), Visibilite = "PRIVE", OrganisateurMatricule = "G001", Statut = "INCOMPLET" });

        var resultat = await _service.RejoindreMatchPublicAsync(1, "L001");
    ...
    public async Task RejoindreMatchPublicAsync_MatchDejaCommence_RetourneEchec() {
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddHours(-1), Visibilite = "PUBLIC", OrganisateurMatricule = "G001", Statut = "INCOMPLET" });
    ...
    public async Task RejoindreMatchPublicAsync_MembreSiteSurUnAutreSite_RetourneEchec() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("S003")).ReturnsAsync(MembreValide("S003", "SITE", 2, 14));
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(1), Visibilite = "PUBLIC", OrganisateurMatricule = "G001", Statut = "INCOMPLET" });

        var resultat = await _service.RejoindreMatchPublicAsync(1, "S003");
    ...
    public async Task RejoindreMatchPublicAsync_MembreSiteSurSonPropreSite_Autorise() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("S001")).ReturnsAsync(MembreValide("S001", "SITE", 1, 14));
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(30), Visibilite = "PUBLIC", OrganisateurMatricule = "G001", Statut = "INCOMPLET" });

        var resultat = await _service.RejoindreMatchPublicAsync(1, "S001");
    ...
    public async Task RejoindreMatchPublicAsync_MembreLibreSansLimiteDeDelai_Autorise() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("L001")).ReturnsAsync(MembreValide("L001", "LIBRE", null, 5));
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(60), Visibilite = "PUBLIC", OrganisateurMatricule = "G001", Statut = "INCOMPLET" });
    ...
    public async Task RejoindreMatchPublicAsync_DetteActive_NEmpecheJamaisEtEstReglee() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("G002")).ReturnsAsync(MembreValide("G002", "GLOBAL", null, 21));
        _detteRepoMock.Setup(r => r.GetNonSoldeeAsync("G002")).ReturnsAsync(new Dette { Id = 7, MembreMatricule = "G002", MatchOrigineId = 5, Montant = 30.00m, Soldee = false });
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(1), Visibilite = "PUBLIC", OrganisateurMatricule = "G001", Statut = "INCOMPLET" });

        var resultat = await _service.RejoindreMatchPublicAsync(1, "G002");

        Assert.True(resultat.Succes);
        Assert.True(resultat.DetteReglee);
        Assert.Equal(45.00m, resultat.MontantPaye); // 15€ + 30€ de dette reportée
        _matchRepoMock.Verify(r => r.InscrireEtPayerAsync(1, "G002", It.Is<Dette>(d => d.Id == 7)), Times.Once);
    }
    ...
    public async Task RejoindreMatchPublicAsync_PenaliteActive_NEmpechePasLInscription() {
        _penaliteRepoMock.Setup(r => r.GetPlusRecenteAsync("G001")).ReturnsAsync(new Penalite { MembreMatricule = "G001", MatchOrigineId = 1, DelaiJusquAu = Aujourdhui.AddDays(30) });
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(1), Visibilite = "PUBLIC", OrganisateurMatricule = "G002", Statut = "INCOMPLET" });

        var resultat = await _service.RejoindreMatchPublicAsync(1, "G001");
    ...
    public async Task RejoindreMatchPublicAsync_MatchComplet_RetourneEchec() {
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(1), Visibilite = "PUBLIC", OrganisateurMatricule = "G001", Statut = "INCOMPLET" });
        _matchRepoMock.Setup(r => r.InscrireEtPayerAsync(1, "L001", It.IsAny<Dette>())).ThrowsAsync(new MatchCompletException());
    ...
    public async Task RejoindreMatchPublicAsync_DejaInscrit_RetourneEchec() {
        _matchRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(
            new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(1), Visibilite = "PUBLIC", OrganisateurMatricule = "G001", Statut = "INCOMPLET" });
        _matchRepoMock.Setup(r => r.InscrireEtPayerAsync(1, "L001", It.IsAny<Dette>())).ThrowsAsync(new DejaInscritException());
    ...
    public async Task ObtenirMontantAPayerAsync_SansDette_Retourne15() {
        var resultat = await _service.ObtenirMontantAPayerAsync("G001");
    ...
    public async Task ObtenirMontantAPayerAsync_AvecDette_ReporteLeMontant() {
        _detteRepoMock.Setup(r => r.GetNonSoldeeAsync("G001")).ReturnsAsync(new Dette { Id = 1, MembreMatricule = "G001", MatchOrigineId = 1, Montant = 45.00m, Soldee = false });
    ...
    public async Task PayerParticipationAsync_ParticipationIntrouvable_RetourneEchec() {
        ...
        var resultat = await _service.PayerParticipationAsync(99, "G001");
    ...
    public async Task PayerParticipationAsync_ParticipationDUnAutreMembre_RetourneEchec() {
        _matchRepoMock.Setup(r => r.GetParticipationByIdAsync(1)).ReturnsAsync(
            new Participation { Id = 1, MatchId = 1, MembreMatricule = "L001", DateInscription = DateTime.Now });

        var resultat = await _service.PayerParticipationAsync(1, "G001");
    ...
    public async Task PayerParticipationAsync_RequeteValideSansDette_Retourne15() {
        var participation = new Participation { Id = 1, MatchId = 1, MembreMatricule = "G001", DateInscription = DateTime.Now };
    ...
    public async Task PayerParticipationAsync_AvecDette_LaRegleEtReporteLeMontant() {
        var participation = new Participation { Id = 1, MatchId = 1, MembreMatricule = "G001", DateInscription = DateTime.Now };
        var dette = new Dette { Id = 7, MembreMatricule = "G001", MatchOrigineId = 5, Montant = 45.00m, Soldee = false };
        _matchRepoMock.Setup(r => r.GetParticipationByIdAsync(1)).ReturnsAsync(participation);
        _detteRepoMock.Setup(r => r.GetNonSoldeeAsync("G001")).ReturnsAsync(dette);
    ...
    public async Task ObtenirParticipationsEnAttenteAsync_MembreConnu_RetourneLesParticipationsTrieesParDate() {
        ...
        var matchProche = new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Today.AddDays(2), Visibilite = "PRIVE", OrganisateurMatricule = "S001", Statut = "INCOMPLET", Site = site, Terrain = terrain };
        var matchLointain = new Match { Id = 2, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Today.AddDays(5), Visibilite = "PRIVE", OrganisateurMatricule = "S002", Statut = "INCOMPLET", Site = site, Terrain = terrain };
        var participations = new List<Participation> {
            new() { Id = 20, MatchId = 2, MembreMatricule = "G001", DateInscription = DateTime.Now, Match = matchLointain },
            new() { Id = 10, MatchId = 1, MembreMatricule = "G001", DateInscription = DateTime.Now, Match = matchProche }
        };
        _matchRepoMock.Setup(r => r.GetParticipationsEnAttenteAsync("G001")).ReturnsAsync(participations);

        var resultat = await _service.ObtenirParticipationsEnAttenteAsync("G001");
        ...
        Assert.Equal("S001", resultat[0].OrganisateurMatricule);
    ...
    public async Task ObtenirReservationsAsync_MembreConnu_RetourneTrieesParDateDecroissanteAvecRoleOrganisateur() {
        ...
        var matchOrganise = new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Today.AddDays(-3), Visibilite = "PRIVE", OrganisateurMatricule = "G001", Statut = "TERMINE", Site = site, Terrain = terrain };
        var matchParticipe = new Match { Id = 2, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Today.AddDays(3), Visibilite = "PUBLIC", OrganisateurMatricule = "S001", Statut = "INCOMPLET", Site = site, Terrain = terrain };
        _matchRepoMock.Setup(r => r.GetReservationsAsync("G001")).ReturnsAsync(new List<Match> { matchOrganise, matchParticipe });

        var resultat = await _service.ObtenirReservationsAsync("G001");
    ...
    public async Task ObtenirReservationsAsync_MatchPasseNonScelle_AfficheTermineCalcule() {
        var site = new Site { Id = 1, Nom = "Site 1" };
        var terrain = new Terrain { Id = 11, SiteId = 1, Numero = 3 };
        var matchPasse = new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddDays(-1), Visibilite = "PRIVE", OrganisateurMatricule = "G001", Statut = "INCOMPLET", Site = site, Terrain = terrain };
        _matchRepoMock.Setup(r => r.GetReservationsAsync("G001")).ReturnsAsync(new List<Match> { matchPasse });
    ...
    public async Task ObtenirReservationsAsync_MatchDansLeCreneauEnCours_GardeLeStatutBrut() {
        ...
        var site = new Site { Id = 1, Nom = "Site 1" };
        var terrain = new Terrain { Id = 11, SiteId = 1, Numero = 3 };
        var matchEnCours = new Match { Id = 1, SiteId = 1, TerrainId = 11, DateHeure = DateTime.Now.AddMinutes(-30), Visibilite = "PRIVE", OrganisateurMatricule = "G001", Statut = "COMPLET", Site = site, Terrain = terrain };
        _matchRepoMock.Setup(r => r.GetReservationsAsync("G001")).ReturnsAsync(new List<Match> { matchEnCours });
    ...
    private static Match MatchDetailValide(string visibilite = "PRIVE", string organisateur = "G001", int siteId = 1, List<Participation>? participations = null) => new() {
        Id = 1,
        SiteId = siteId,
        TerrainId = 11,
    ...
    public async Task ObtenirDetailAsync_MatchInconnu_RetourneNull() {
        ...
        var resultat = await _service.ObtenirDetailAsync(99, "G001");
    ...
    public async Task ObtenirDetailAsync_Organisateur_RetourneLeDetail() {
        var match = MatchDetailValide(organisateur: "G001");
        _matchRepoMock.Setup(r => r.GetDetailAsync(1)).ReturnsAsync(match);

        var resultat = await _service.ObtenirDetailAsync(1, "G001");
    ...
    public async Task ObtenirDetailAsync_Participant_RetourneLeDetailAvecStatutDePaiement() {
        var participations = new List<Participation> {
            new() { MembreMatricule = "G001", DateInscription = DateTime.Now, Paiement = new Paiement { MontantParticipation = 15.00m, MontantDetteReportee = 0.00m, DatePaiement = DateTime.Now } },
            new() { MembreMatricule = "L001", DateInscription = DateTime.Now } // en attente
        };
        var match = MatchDetailValide(organisateur: "S001", participations: participations);
        _matchRepoMock.Setup(r => r.GetDetailAsync(1)).ReturnsAsync(match);

        var resultat = await _service.ObtenirDetailAsync(1, "L001");

        Assert.NotNull(resultat);
        Assert.True(resultat!.Joueurs.Single(j => j.MembreMatricule == "G001").Paye);
        Assert.False(resultat.Joueurs.Single(j => j.MembreMatricule == "L001").Paye);
    }

    [Fact]
    public async Task ObtenirDetailAsync_NonImpliqueMatchPrive_RetourneNull() {
        var match = MatchDetailValide(visibilite: "PRIVE", organisateur: "S001");
        _matchRepoMock.Setup(r => r.GetDetailAsync(1)).ReturnsAsync(match);
    ...
    public async Task ObtenirDetailAsync_NonImpliqueMatchPublicGlobal_RetourneLeDetail() {
        var match = MatchDetailValide(visibilite: "PUBLIC", organisateur: "S001", siteId: 2);
        _matchRepoMock.Setup(r => r.GetDetailAsync(1)).ReturnsAsync(match);
    ...
    public async Task ObtenirDetailAsync_NonImpliqueMembreSiteMemeSite_RetourneLeDetail() {
        _membreRepoMock.Setup(r => r.GetByMatriculeAsync("S003")).ReturnsAsync(MembreValide("S003", "SITE", 1, 14));
        var match = MatchDetailValide(visibilite: "PUBLIC", organisateur: "G001", siteId: 1);
        _matchRepoMock.Setup(r => r.GetDetailAsync(1)).ReturnsAsync(match);

        var resultat = await _service.ObtenirDetailAsync(1, "S003");
