...
namespace PadelManager.WinForms
{
    ...
    public partial class CreerMatchPublicForm : Form {
        ...
        public CreerMatchPublicForm(ConnexionResultat connexion) {
            InitializeComponent();
        ...
        private async void CreerMatchPublicForm_Load(object sender, EventArgs e) {
        ...
        private async void btnCreer_Click(object sender, EventArgs e) {
            ...
            var requete = new CreerMatchPublicRequete {
            ...
            try {
                var resultat = await _apiClient.CreerMatchPublicAsync(requete);

                if (resultat.Succes) {
                    // Rafraîchit la liste (le créneau n'est plus libre) AVANT le message de
                    // confirmation, sinon RechercherAsync l'écrase aussitôt.
                    await RechercherAsync();
                    lblMessage.Text = $"Match public créé (terrain {creneau.NumeroTerrain}, {requete.Date:dd/MM/yyyy} {requete.HeureDebut}) et votre part de 15€ payée. Les 3 autres places sont désormais ouvertes.";
                } else {
